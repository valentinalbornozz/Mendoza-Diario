    ...
    const cargarPortadas = async () => {
      ...
      for (const noticia of noticias) {
        try {
          ...
          console.error(`Error al obtener la portada para la noticia ${noticia.id}:`, error);
        }
      }
    ...
    fetch(`http://localhost:8080/api/noticia/eliminar/${id}`, {
      ...
      .then(async response => {
        if (response.ok) {
          ...
          setNoticias(prevAutores => prevAutores.filter(autor => autor.id !== id));
        } else {
          setNotificationMessage('Error al eliminar la noticia. Response not ok: ' + response.statusText);
          setShowNotification(true);
        }
      })
      .catch(error => {
        ...
        <tbody>
          {noticias.map((noticia) => (
            <tr key={noticia.id}>
              ...
              <td>{noticia.autor.nombre} {noticia.autor.apellido}</td>
